...
对于定义在同一支撑集 $\mathcal{X}$ 上的离散分布 $P$ 与 $Q$，其中 $P(x)$ 和 $Q(x)$ 分别表示各分布对结果 $x$ 所赋予的概率，从 $Q$ 到 $P$ 的 KL 散度定义为
...
对于两个对角高斯分布这一在变分方法中反复出现的特殊情形，KL 散度具有解析闭式表达。设 $q = \mathcal{N}(\mu, \mathrm{diag}(\sigma^2))$ 为 $d$ 维对角高斯分布，其均值为 $\mu \in \mathbb{R}^d$，各维度方差为 $\sigma_j^2$，$p = \mathcal{N}(0, I)$ 为标准正态先验，则
...
其中 $\rho^{\pi_\mathrm{old}}$ 为旧策略 $\pi_\mathrm{old}$ 所诱导的状态访问分布，$s$ 和 $a$ 分别为状态与动作，$A^{\pi_\mathrm{old}}$ 为优势函数，$\delta$ 为信任域半径。KL 约束确保新策略保持在旧策略的邻域内，在该邻域中，重要性加权目标是真实性能提升的可靠估计。Schulman 等人在此约束下证明了单调改进保证，使 TRPO 成为首个具有可证明安全更新规则的策略梯度方法。
...
其中 $x$ 为从训练分布 $\mathcal{D}$ 中采样的输入提示，$y$ 为模型生成的回复，$r(x, y)$ 为奖励模型的评分，$\beta$ 为在奖励最大化与分布保真度之间权衡的系数。当 $\beta = 0$ 时策略无约束并退化；随着 $\beta$ 增大，策略更接近参考模型，但可能对奖励信号欠拟合。实践中通过调整 $\beta$ 找到一个区间，使模型在提升人类偏好指标的同时仍保持连贯的语言生成能力。该目标中的 KL 项在形式上与 TRPO 中的相同，但作用不同：它不是保证策略安全改进，而是防止不完美奖励建模所诱发的分布偏移。
