...
$$\text{tIoU} = \frac{|\text{prediction} \cap \text{ground truth}|}{|\text{prediction} \cup \text{ground truth}|}$$ A predicted segment is considered a true positive only if its tIoU with a ground truth segment exceeds the threshold and the predicted class is correct. Average Precision (AP) is computed per class by ranking all predictions by confidence and computing the area under the precision-recall curve. mAP averages AP over all classes.
