...
### Q8 [进阶] 分析 bias-variance 分解与双下降现象

**Q：** 经典的 bias-variance 权衡与现代过参数化模型中观察到的双下降现象有何关联？

**A：** **bias-variance decomposition**（bias-variance 分解）将模型 $\hat{f}$ 在点 $x$ 处的期望测试误差表示为：
...
bias 度量模型假设带来的系统误差；variance 度量对特定训练集的敏感程度。经典图像预测测试误差呈 U 形曲线：小模型欠拟合（高 bias、低 variance），大模型过拟合（低 bias、高 variance），最优复杂度使二者之和最小。
...
双下降现象已在线性回归、核方法、随机森林和深度神经网络中被观测到（Belkin et al., 2019）。这意味着，在过参数化体制下，经典的 bias-variance 框架是不完整的；当容量充足时，通过早停或显式权重惩罚进行正则化可能是不必要的，甚至有害的。该现象还取决于插值算法的选择：最小范数最小二乘表现出干净的双下降，而其他插值算法则未必。
...
| Q8 | 进阶 | bias-variance 分解与双下降现象 | 概率与统计 |
